...
namespace Albedo {
 ...
 namespace Runtime
 {
        ...
        std::optional<bool> RegisterEvent::sm_result{};
        std::string RegisterEvent::sm_feed_back{};

        void RegisterEvent::sendUserInfo(std::string name, std::string account, std::string password)
        {
            if (!isTriggered())
            {
                if (!g_context_net.isOnline) return RegisterEvent::trigger(false, "Offline");
        ...
        void RegisterEvent::trigger(bool result, std::string feedback)
        {
            if (isTriggered())
        ...
        EventResult RegisterEvent::getResult()
        { 
            if (!isTriggered())
